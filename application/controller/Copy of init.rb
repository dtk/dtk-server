...
#select field for saved search dropdown is coded into view render search function
        ...
        search_id = (search_object[:id].nil?) ? model_name().to_s+'-list-new' : model_name().to_s+'-list-'+search_object[:id].to_s

        tpl.assign("_saved_search_list",_saved_search_list)
        (_saved_search_list||[]).each do |so|
          add_js_exe("R8.Search.setSearchObj(#{JSON.generate(so)});")
        end

        if(search_object[:id])
          add_js_exe("R8.Search.loadSearchObj('#{search_object[:id]}');")
          add_js_exe("R8.Search.initSearch('#{search_object[:id]}','#{search_id}');")
          tpl.assign(:search_guid,search_id)
        end

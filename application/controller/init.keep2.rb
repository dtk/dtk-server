...
#TODO: factor in pagining and ordering ?query_params={'start_at':21,'ordering':[{'field':'status','order':'DESC'}]}
#changing tyle to {:order_by => {:field_name=> "ASC"}}
      order_by = [{:field => :display_name, :order => "ASC"}]

      parent_id = where_clause ? where_clause.delete(:parent_id) : nil

      field_set = Model::FieldSet.default(model_name())
      where_clause = field_set.ret_where_clause_for_search_string(request.params)

      _model_var = {}
      _model_var[:i18n] = get_model_i18n(model_name().to_s,user_context())

      #TODO: temp hack to convert from ' to " in query params
      query_params = (request.params['query_params'].nil? or request.params['query_params'].empty?) ? {} : JSON.parse(request.params['query_params'].gsub(/'/,'"'))
...
#TODO hack that for testing, which no only implemented by node
      ...
      if model_name() == :node
        ...
        tpl.assign("#{model_name().to_s}_current_start",start)
        tpl.assign(:_app,_app)
        model = {}

        field_set.cols.each do |field|
          model[field] = (request.params[field.to_s].nil? or request.params[field.to_s].empty?) ? '' : request.params[field.to_s]
        end
...
#TODO: figure out where to put, setting the sort vars for each field/column
      order_by_list = {}
      if !query_params['order_by'].nil?
        query_params['order_by'].each do |order_item|
          order_by_list[order_item['field'].to_sym] = order_item['order']
        end
      end
      field_set.cols.each do |field|
#        sort_order = (!query_params['order_by'].nil? && query_params['order_by'][field.to_s] && query_params['order_by'][field.to_s] == 'ASC' ? 'DESC' : 'ASC')
        sort_order = (order_by_list[field] && order_by_list[field] == 'ASC') ? 'DESC' : 'ASC'
        if order_by_list.empty? || order_by_list[field].nil?
          sort_class = ''
        else
          sort_class = (sort_order == 'DESC') ? 'asc' : 'desc'
        end

        tpl.assign((field.to_s+'_order').to_sym,sort_order)
#        sort_class = (query_params['order_by'].nil? || query_params['order_by'][field.to_s].nil?) ? '' : sort_order.downcase 
        tpl.assign((field.to_s+'_order_class').to_sym,sort_class)
      end

      limit = R8::Config[:page_limit] || 20
      limit = 10
      num_model_items = 10000
...
#TODO: what is parent id needed for?
...
#TODO: helper fn; put heer so otehr controllers can use it; move this
def _app
  {
#        :js_includes => js_includes,
#        :css_includes => css_includes,
...
%w{workspace datacenter node_group node node_interface component attribute attribute_link monitoring_item}.each do |controller_file|

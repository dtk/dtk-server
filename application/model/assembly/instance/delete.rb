...
module DTK; class  Assembly
  class Instance
    ...
    module DeleteMixin
      require_relative('delete/components')

      def delete_by_path(path)
        delete_adapter, params = ret_adapter_and_params_from_path(path)
        delete_class = Module.nesting.first.const_get("#{capitalize_adapter_name(delete_adapter)}")
        # delete_class = load_for("DeleteMixin::#{delete_adapter}")

...
module DTK; class Assembly
  class Template < self
    ...
    def stage(target, opts = Opts.new)
      ...
      # TODO: wil deprecate opts[:assembly_name] for :service_name
      if service_name = opts[:service_name] || opts[:assembly_name]
        fail ErrorUsage.new("Service '#{service_name}' already exists in target '#{target.get_field?(:display_name)}'") if Assembly::Instance.exists?(target, service_name)

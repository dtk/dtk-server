    ...
    class ArrayAppend < Base
      ...
      def internal_hash_form(opts = {})
        ...
        if @index_map.nil? && (@input_path.nil? || @input_path.empty?)
          ...
          Output::ArrayAppend.new(array_slice: new_rows, attr_link_id: @attr_link_id, output_is_array: output_is_array)
        else
          index_map_persisted = @index_map ? true : false
          index_map = @index_map || IndexMap.generate_from_paths(@input_path, nil)
          Output::Partial.new(attr_link_id: @attr_link_id, output_value: output_value, index_map: index_map, index_map_persisted: index_map_persisted)
        end
      end

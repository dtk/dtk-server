  ...
  class Component
    class ResourceMatching
      ...
      def self.find_matches_and_conflicts(cmp_ref_mh,node_id_cmp_ref_mapping)
        ...
        ndx_cmp_attr_info = get_ndx_cmp_attr_info(cmp_ref_mh,target_node_ids,cmp_refs)
     ...
     private
      ...
      def self.get_ndx_cmp_attr_info(cmp_ref_mh,target_node_ids,cmp_refs)
        ret = Hash.new
        ...
        Model.get_objs(cmp_mh,sp_hash).each do |cmp|
          cmp_id = cmp[:id]
          pntr = ret[cmp_id] ||= cmp.hash_subset(:id,:group_id,:display_name).merge(:attributes => Array.new)

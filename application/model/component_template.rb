...
module XYZ
  module ComponentTemplate
    def update_default(attribute_name,val,field_to_match=:display_name)
      tmpl_attr_obj =  get_virtual_attribute(attribute_name,cols,field_to_match)
      ...
      cmp_ids_to_update = Model.get_objects_from_join_array(model_handle,base_sp_hash,join_array).map{|r|r[:id]}
      unless cmp_ids_to_update.empty?
        filter = [:oneof, :id, cmp_ids_to_update]
        Model.update_rows_meeting_filter(model_handle,{:value_asserted => val},filter)
      end
    end
  end
end

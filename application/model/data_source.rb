...
module XYZ
  class DataSource < Model
    ...
    class << self
      ...
      def create(container_handle_id,ref,hash_content={})
        ...
        hash_with_defaults = fill_in_defaults(hash_content)
     ...
     private
      ...
      def fill_in_defaults(hash_content)
        ...
        if hash_with_defaults[:data_source_object]
          hash_with_defaults[:data_source_object].each do |k,child_hash_content|
             hash_with_defaults[:data_source_object][k.to_s] = 
              DataSourceObject.fill_in_defaults(k.to_sym,child_hash_content)
          end
        end
        hash_with_defaults
      end
  ...
  class DataSourceObject < Model
    ...
    class << self
      def up()
        column :object_type, :varchar, :size => 25 
      ...
      #helper fns
      DS_object_defaults = {}
      def fill_in_defaults(object_type,hash_content)
        hash_with_defaults = Hash.new
        [:filter,:update_policy,:polling_policy,:objects_location].each do |key|
          v = hash_content[key] || Aux.nested_value(DS_object_defaults,[object_type,key ])
          hash_with_defaults[key] = v if v
        end
        hash_with_defaults[:object_type] = object_type.to_s
        hash_with_defaults
      end
    end
  end

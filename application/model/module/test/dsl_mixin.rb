module DTK; class TestModule
  module DSLMixin
    ...
    def create_new_dsl_version(new_dsl_integer_version,format_type,module_version)
      ...
      component_dsl = TestDSL.create_dsl_object(module_branch,previous_dsl_version)
    ...
    def parse_dsl_and_update_model(impl_obj,module_branch_idh,version=nil,opts={})
      ...
      TestDSL.parse_and_update_model(self,impl_obj,module_branch_idh,version,opts)
      set_dsl_parsed!(true)
    end
...
Log.info_pp(["Using deprecate_create_needed_objects_and_dsl?; local =",local,caller[0..4]])
    ...
    def create_needed_objects_and_dsl?(repo,local,opts={})
      ...
      if TestDSL.contains_dsl_file?(impl_obj)
        if e = TestDSL::ParsingError.trap{parse_dsl_and_update_model(impl_obj,module_branch_idh,version,opts)}
    ...
    def update_model_objs_or_create_dsl?(diffs_summary,module_branch,version,opts={})
      ...
      elsif TestDSL.contains_dsl_file?(impl_obj)
        if opts[:force_parse] or diffs_summary.meta_file_changed?() or (get_field?(:dsl_parsed) == false)
          if e = TestDSL::ParsingError.trap{parse_dsl_and_update_model(impl_obj,module_branch.id_handle(),version,opts)}
            ret.merge!(:dsl_parsed_info => e)
          end
        end
      else
    ...
    def parse_impl_to_create_dsl(config_agent_type,impl_obj,opts={})
      ...
      begin
        ...
        dsl_generator = TestDSL::GenerateFromImpl.create()
      ...
      if render_hash
        ...
        dsl_filename = TestDSL.dsl_filename(config_agent_type,format_type)

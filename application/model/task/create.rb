...
=begin
      ...
      through all nodes in the nodes_to_create, then:
        In every iteration I check if value of node_group parameter that was passed(catapult_node in this case) 
        matches the display_name `catapult_node:x` (where x is the index of a node in node group - I just do the match check for a substring before `:`)
        If it does I create a hash and merge it to task in the same way as the above mentioned method does
      
      What do you think about this approach?

      If node was passed as a parameter and not a node group, I think in that case I only need to change 
      the node hash. 
      That can be done straight forward somewhere in the flow you showed in the previous comment. 
      Although, I do not have an example in the module for a parameter that is only a node 
...
=begin
      ...
      ...
        @component = {
                           :id => 2147755480,
                     :group_id => 2147484269,
                 :display_name => "wf__on_node_group[default]",
               :component_type => "wf__on_node_group",
            :implementation_id => 2147755409,
                   :basic_type => "service",
                      :version => "master",
            :only_one_per_node => false,
                ...
                :type => "bash_commands"
                 ...
                 :node_node_id => 2147755464,
                :extended_base => nil,
                  :ancestor_id => 2147755426,
                  :assembly_id => 2147755462,
                         :node => {
                                   :id => 2147755464,
                         :display_name => "ng",
                             :group_id => 2147484269,
                ...
                [0] {
                              ...
                              :display_name => "create",
                                  :group_id => 2147484269,
                    :component_component_id => 2147755426,
                                   :content => {
                        :commands => [
                            [0] "RUN echo on_node_group"
                        ],
                        :provider => "bash_commands"
                    }
                }
            ],
                       :source => {
      ...
      #### Notable section of above hash is: ####
                :node => {
                                   :id => 2147755464,
                         :display_name => "ng",
                             :group_id => 2147484269,
      ...
      (from database) node ref is:  assembly--wf-create--ng
      
      Comparing this to task_template_content of dtk exec-s workflow_test_catapult in tenant service instance:

      @component = {
                     :id => 2147754363,
               :group_id => 2147484269,
           :display_name => "kubernetes_local_storage[catapult_node]",
         :component_type => "kubernetes_local_storage",
      :implementation_id => 2147700239,
             :basic_type => "service",
                :version => "master",
      :only_one_per_node => false,
           :external_ref => {
          :entrypoint => "bin/cluster/compute_mount_point_info.rb",
            :provider => "dynamic",
                :type => "ruby"
      },
           :node_node_id => 2147754335,
          :extended_base => nil,
            :ancestor_id => 2147700240,
            :assembly_id => 2147754332,
                   :node => {
                             :id => 2147751333,
                           :type => "staged",
                   :display_name => "master",
          ...
          [0] {
                         :method_name => "create_workflow",
                                  :id => 2147706279,
                        :display_name => "create_workflow",
                            :group_id => 2147484269,
              :component_component_id => 2147700240,
                             :content => {
                  :provider => "workflow",
                      :type => "workflow",
                  :workflow => {
                      :subtasks => [
                          [0] {
                              :components => [
                                  [0] {
                                      :"kubernetes_local_storage::mount[{{name}}]" => {
                                          :node_group => {
                                              :"{\"node_group\"=>nil}" => nil
                                          }
                                      }
                                    ...
                                    :name => "mount local storage for {{name}}"
                          },
                          [1] {
                              :components => [
                                  [0] "kubernetes_local_storage::config[{{name}}]"
                              ],
                                    :name => "create kube objects for local persistent storage for {{name}}"
                          }
                      ]
                  }
              }
        ...
        :configured_node => {
          ...
          :object => {
                        :id => 2147754332,
              :display_name => "catapult_tenant-tenant"
              }
          }
      }
      ...
      (from database) node ref is: assembly--kubernetes_base_cluster-local_storage--master - which is (I suppose) not what we want

      What I thought was I could change the node hash here. However, we do not have info of catapult_node node group here, we only have master.
      The master node is linked with the component in database - this is linked earlier (either in stage or install).
      So I decided to find another solution. 

      EDIT: What (I think) could solve this problem is if we can somehow get the following node from database:
        - Taken from database (node.node):
          id = 2147702853
          ref = test_ns:catapult_tenant-tenant--catapult_node
          display_name = catapult_node
          ...
      


      In below comments you suggested that the change might also be done in the following snippet:
...
=>  99:               node_id = a.node_id
   ...
   a.node_id here is 2147751333 which ties to the problem I mentioned above. We do not have info for node group catapult_node
    
   What we do have is a list of all nodes saved in nodes_to_create here:
    
    In application/model/task/create.rb
    172: nodes_to_create, nodes_wait_for_start = nodes_to_process_in_task(assembly, Aux.hash_subset({}, [:start_nodes, :ret_nodes_to_start]))
  
   catapult_node:1, catapult_node:2, mongo:1  
   
   So basically the question for the above context is am I looking at the right place? And is there something I am overlooking there?
    
   
   I left further comments in
   
    [22, 31] in /home/dtk1/server/current/application/model/task/create.rb
   22:       unless task = Create.create_from_assembly_instance?(assembly, opts)
   23:         return ret
   24:       end
   25:       task[:breakpoint] = opts[:breakpoint] if opts[:breakpoint]
   26:       #alters task if needed to decompose node groups into nodes
=> 27:       NodeGroupProcessing.decompose_node_groups!(task)
   28:     end


=end
      task_template_content.each do |task_template|
        task_template.replace_workflow_info(full_workflow, nodes_to_create)
      end
...
=begin
  ...
  Code above only serves as an example of modifying task_template_content after it has been formed.
  What we end up with is a hash that I've sent you on slack, I will copy it here:

  {
                     :id => 2147714784,
               :group_id => 2147484269,
           :display_name => "kubernetes_local_storage[mongo]",
         :component_type => "kubernetes_local_storage",
      :implementation_id => 2147700239,
             :basic_type => "service",
                :version => "master",
      :only_one_per_node => false,
          ...
          :entrypoint => "bin/cluster/compute_mount_point_info.rb",
           ...
           :node_node_id => 2147714757,
          :extended_base => nil,
            :ancestor_id => 2147700240,
            :assembly_id => 2147714756,
                   :node => {
                                :id => 2147714956,
                          :group_id => 2147484269,
                      :display_name => "mongo:1",
                           :os_type => nil,
                              :type => "target_ref_staged",
                       :description => nil,
                                :ui => nil,
                      :external_ref => {
              :image_id => nil,
          ...
          :datacenter_datacenter_id => 2147714755,
                            :target => {
                           :id => 2147714755,
                     :group_id => 2147484269,
                 :display_name => "catapult-tenant-tenant-final",
              :iaas_properties => {}
          },
                             :index => "1",
                 :ng_member_deleted => nil
      },
                  :title => "mongo",
            :action_defs => [
          [0] {
                  ...
                  :workflow => {
                      :subtasks => [
                          [0] {
                              :components => [
                                  [0] {
                                      :"kubernetes_local_storage::mount[mongo]" => {
                                          :node_group => "mongo"
                                      }
                                  }
                              ],
                                    :name => "mount local storage for mongo"
                          },
                          [1] {
                              :components => [
                                  [0] "kubernetes_local_storage::config[mongo]"
                              ],
                                    :name => "create kube objects for local persistent storage for mongo"
                          }
                      ]
                  }
              }
  ...
  I replaced the node with mongo:1 node and action_def with substituted templates.

  I suppose the problem with this hash is that the `node` hash is not formed correctly as it should be formed as
  a node group (hierarchical task example you showed in jira) and not as a node?
  
  Also, should I leave action_defs as is and is there anything else that needs to be done with it?

=end

      stages_config_nodes_task = task_template_content.create_subtask_instances(task_mh, assembly.id_handle())

    #  serialized_result = get_serialized_content_from_assembly(assembly, , )

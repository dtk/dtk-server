...
if (!R8.IDE.View.editor_target.node) {
	...
	R8.IDE.View.editor_target.node = function(node,viewSpace) {
		var _node = node,

//TODO: revisit, id/type/object are needed/shared between tree and workspace, need to merge behavior to one
			_id = (typeof(_node.id) == 'undefined') ?  _node['object']['id'] : _node.id,
			_object = _node['object'],
			_type = _node['type'],

			_os_type = _node.os_type,
			_dataModel = null,
			_ports = {},
			_portDefs = null,
			_portsReady = false,
...
//TODO: revisit.., link creation depends on this for drop setup/detection
		...
		return {
			...
			renderTree: function() {
				var nodeLeaf = {
					'node_id': 'target-'+_node.id,
					'type': 'node-'+_os_type,
					'basic_type': 'node',
					'name': _node.display_name
//					'name': _node.name
				};
				...
				var applicationsLeaf = {
					'node_id': 'node-applications-'+_node.id,
					'type': 'applications',
					'basic_type': '',
					'name': 'Applications'
				};
				...
				for(var c in _node.components) {
					var componentId = _node.components.id;
					_applications[componentId] = new R8.Component(_node.components[c]);
...
//TODO: make this some config param
				...
				for (i in _portDefs) {
//DEBUG
//console.log('Rendering portDef:');
//console.log(_portDefs[i]);

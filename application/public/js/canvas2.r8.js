...
//				var endConnectorFacing = linkDef.endItems[0].location;
				...
				if (tempCanvas != null) {
					...
					R8.Utils.Y.one('#'+containerNodeId).appendChild(canvasNode);
				}
				delete (tempCanvas);
				//---------------------------------------------------------------------
...
//				var endElemXY = endElemNode.getXY();
				...
				//DEBUG
				var wspaceNode = R8.Utils.Y.one('#'+containerNodeId);
...
//				var canvasClass = connectorType + '-' + ;
			 ...
			 * @param {Node} dragNode
			 */
			renderDragWire: function(startElemNode,dragElemNode,portDef,containerNodeId) {
...
var viewSpaceLeft = R8.Utils.Y.one('#'+containerNodeId).get('region').left;
var viewSpaceTop = R8.Utils.Y.one('#'+containerNodeId).get('region').top;
				...
				if(wireCanvasElem === null) {
					...
					R8.Utils.Y.one('#'+containerNodeId).appendChild(canvasElem);
				} else {
...
//							var canvasTop = startElemXY[1] - ctrlYPtOffset;
							var canvasLeft = startElemXY[0] - viewSpaceLeft;
...
//							var canvasTop = startElemXY[1];
							var canvasLeft = startElemRegion['left']-viewSpaceLeft;
						...
						case 'west':
							...
							var canvasActualHeight = canvasBaseHeight;
							var canvasLeft = startElemXY[0] - ctrlXPtOffset - viewSpaceLeft;
							var canvasTop = startElemXY[1] - viewSpaceTop;
...
//							var canvasTop = startElemXY[1] - ctrlYPtOffset;
							var canvasLeft = dragElemXY[0] - viewSpaceLeft;
							var canvasTop = startElemRegion['top'] - (ctrlYPtOffset+viewSpaceTop);
...
//							var canvasLeft = dragElemXY[0];
//							var canvasTop = startElemXY[1];
							var canvasLeft = dragElemRegion['left'] - viewSpaceLeft;
							var canvasTop = startElemRegion['top']-viewSpaceTop;
...
//							var canvasTop = dragElemXY[1];
				...
				for (var i in R8.Canvas2.config.dragWire) {
					var connectorCfg = R8.Canvas2.config.dragWire[i];
					ctx.beginPath();
					ctx.strokeStyle = connectorCfg.strokeStyle;
					ctx.lineWidth = connectorCfg.lineWidth;
					ctx.lineCap = connectorCfg.lineCap;

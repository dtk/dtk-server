...
*/
				...
				_rootNode = R8.Utils.Y.Node.create('<ul>');
				_rootNode.set('id','task-'+taskDef.task_id+'-container');
				_rootNode.addClass('filetree');

				this.setTaskContent(taskDef,viewType,_rootNode);
//				R8.Workspace.get('modalNode').append(rootNode);
				R8.Utils.Y.one('#'+panel_id).append(_rootNode);

				$("#"+_rootNode.get('id')).treeview({
					collapsed: true
				});

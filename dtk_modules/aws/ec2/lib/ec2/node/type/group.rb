    ...
    module Type
      class Group < Ec2::Node
        ...
        def start
          ...
          instance_info_array = 
            if instances.size > 0
              aws_operation(:start).start_instances(instance_ids(instances))
            end
        ...
        def stop
          ...
          instance_info_array = 
            if instances.size > 0
              aws_operation(:stop).stop_instances(instance_ids(instances))
            end
        ...
        def create_instances(count)
          ...
          aws_operation(:create).create_instances(count)
        end
        
        def terminate_instances(instance_ids)
          aws_operation_class(:terminate).terminate_instances(client, instance_ids)
        end
        ...
        def describe_instances(instance_ids)
          ...
          aws_operation_class(:get).describe_instances(client, instance_ids)
        end

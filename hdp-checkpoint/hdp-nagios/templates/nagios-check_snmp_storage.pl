...
#################################################################
# This script implements additional function of accepting 
# comma separated list of directories for -m option, when 
# no regex and no negate options are specified. 
# Licence : GPL - http://www.fsf.org/licenses/gpl.txt
#################################################################
#
...
# Check if key matches one of the mount points
sub check_match {
  my $key = shift;
  my $mntpnts_ref = shift;
  foreach my $mnt (keys %$mntpnts_ref) {
    #if ($mnt eq $key) { return 1; }
    if ($mnt =~ $key) { return 1; }
  }
  return 0;
}
...
# If mount point is a comma separated list of dirs
# TODO: Sort the mount points received from SNMP 
# decending length of mount point path. Compare
# directories to get the mountpoints.
my %mount_points = ();
if (defined($o_noreg) && !defined ($o_negate)) {
  my @dirs = split (/,/,$o_descr);
  foreach my $dir (@dirs) {
    $dir =~ s/^\s+//; $dir =~ s/\s+$//; 
    #if (!-d $dir) { next; }
    #my $mntpnt = `df $dir | grep -v "^Filesystem" | awk -F" " '{print \$NF}'`;
    #chomp($mntpnt);
    #print "MNT: <$mntpnt>\n";
    $mount_points{$dir} = "";
  } 
...
foreach my $key ( keys %$resultat) {
   ...
   if (defined($o_negate)) {
     ...
     $test = defined($o_noreg)
                ? check_match($$resultat{$key}, \%mount_points)
                : $$resultat{$key} =~ /$o_descr/;
   }  
  if ($test) {

...
function check_dtk_dsl_version() {
  ...
  if [[ a == b ]]; then
    echo "No change is needed in dtk-client.gemspec"
    echo $current_dtk_dsl_version_in_gemspec
  else
    echo $dtk_dsl_tag
  fi
}

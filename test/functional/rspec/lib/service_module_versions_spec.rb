...
shared_context 'Publish versioned service module' do |dtk_common, service_module_name, remote_service_module_name, version_name|
	it "publish/push service module #{service_module_name} with version #{version_name}" do
	    ...
	    pass = false
	    module_name = component_module_name.split(':')[1] #exract 'module_name' from string 'namespace:module_name'
	    value = `dtk service-module #{component_module_name} publish #{module_name} -v #{version_name}`
	    puts value
	    pass = true if (value.include? 'Status: OK')
	    puts "Publish of service module #{component_module_name} #{version_name} completed successfully!" if pass == true
	    puts "Publish of service module #{component_module_name} #{version_name} was not successfully" if pass == false
	    puts ''
...
shared_context 'NEG - Publish versioned service module' do |dtk_common, service_module_name, remote_service_module_name, version_name|
	it "does not publish/push service module #{service_module_name} since this version #{version_name} does not exist" do
	    ...
	    puts "Publish of service module #{component_module_name} #{version_name} was successfully, even though it should be!y!" if pass == false
	    puts "Publish of service module #{component_module_name} #{version_name} was not successfully" if pass == true
	    puts ''
	    expect(pass).to eq(true)
	end
end

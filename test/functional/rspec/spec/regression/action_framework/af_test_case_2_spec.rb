...
service_instance_location = '~/dtk/af_test_case_2_instance'
...
describe '(Action Framework) Test Case 2: Service with two nodes that contain cmp with action with multiple success commands in it' do
  ...
  context 'Get task status details for action with two successfull commands' do
    include_context 'Get task status details', service_instance_location, "STAGE 5", [expected_output_1, expected_output_2]
  end

  context 'Get task status details for action with two successfull commands' do
    include_context 'Get task status details', service_instance_location, "STAGE 6", [expected_output_1, expected_output_2]
  end

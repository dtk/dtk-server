...
describe "(Component Module DSL) Test Case 12: NEG - Ill-formed yaml content (type: integersss instead of integer) in dtk.model.yaml file and push-clone-changes to server" do
  ...
  context "Set incorrect type value to integersss in dtk.model.yaml file" do
    include_context "Replace dtk.model.yaml file with new one", local_component_module_name, file_for_change_location, file_for_change, component_module_filesystem_location, "sets incorrect type value integersss instead of integer in attribute section in dtk.model.yaml"
  end

...
describe "(Component Module DSL) Test Case 13: dtk.model.yaml with all correct values for supported types (string, integer, boolean, hash, array, port)" do
  ...
  context "Remove existing component in dtk.model.yaml file" do
    include_context "Replace dtk.model.yaml file with new one", local_component_module_name, file_for_change_location, file_for_change, component_module_filesystem_location, "adds all supported types to dtk.model.yaml"
  end

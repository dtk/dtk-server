...
describe "(Component Module DSL) Test Case 14: dtk.model.yaml with empty array and hash type attribute" do
  ...
  context "Check if expected attribute value for attribute exist" do
    include_context "Check if expected attribute value exists for given attribute name", dtk_common, component_module_name, component_name, attribute_name_1, attribute_value
  end

  context "Check if expected attribute value for attribute exist" do
    include_context "Check if expected attribute value exists for given attribute name", dtk_common, component_module_name, component_name, attribute_name_2, attribute_value
  end

...
describe "(Component Module DSL) Test Case 16: dtk.model.yaml with hash type attribute that contains two key value pairs and hash type attribute where key contains array as value" do
  ...
  context "Remove existing component in dtk.model.yaml file" do
    include_context "Replace dtk.model.yaml file with new one", component_module_name, file_for_change_location, file_for_change, component_module_filesystem_location, "adds two hash type attributes to dtk.model.yaml"
  end
  ...
  context "Check if expected attribute value for attribute #{attribute_name_1} exist" do
    include_context "Check if expected attribute value exists for given attribute name", dtk_common, component_module_name, component_name, attribute_name_1, attribute_value_1
  end

  context "Check if expected attribute value for attribute #{attribute_name_2} exist" do
    include_context "Check if expected attribute value exists for given attribute name", dtk_common, component_module_name, component_name, attribute_name_2, attribute_value_2
  end

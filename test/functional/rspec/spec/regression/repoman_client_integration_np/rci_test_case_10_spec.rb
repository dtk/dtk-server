# Test Case 10: NEG - Make public module A (User A is not owner but belongs to User group A which is not set on module, permissions: RWDP/RWDP/R)
...
user_data = {
  ...
  user: '', # To be set later in the script
...
describe '(Repoman client integration) Test Case 10: NEG - Make public module A (User A is not owner but belongs to User group A which is not set on module, permissions: RWDP/RWDP/R)' do
  ...
  context "Usergroup #{user_data[:another_usergroup]}, user #{user_data[:another_user]} and RWDP/RWDP/R permissions" do
  ...
  context 'NEG - Make public component module' do
    include_context 'NEG - Make public component module', dtk_common, component_module
  end

  context "Usergroup #{user_data[:usergroup]}, user #{user_data[:user]} and RWDP/RWDP/R permissions" do
    include_context 'Check module permissions', dtk_common, user_data[:module_name], 'RWDP/RWDP/R'
  end

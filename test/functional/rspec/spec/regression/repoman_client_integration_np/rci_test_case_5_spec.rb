# Test Case 5: NEG - Make private module A (User A is not owner but belongs to User group A which is not set on module, permissions: RWDP/RWDP/RWD)
...
describe "(Repoman client integration) Test Case 5: NEG - Make private module A (User A is not owner but belongs to User group A which is not set on module, permissions: RWDP/RWDP/RWD)" do
	...
	context "Usergroup #{user_data[:another_usergroup]}, user #{user_data[:another_user]} and RWDP/RWDP/RWD permissions" do
  ...
  context "Usergroup #{user_data[:usergroup]}, user #{user_data[:user]} and RWDP/RWDP/RWD permissions" do
  	include_context "Check module permissions", dtk_common, user_data[:module_name], "RWDP/RWDP/RWD"
	end

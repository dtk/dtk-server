# Test Case 8: Make public module A (User A is not owner but belongs to User group A which is not set on module, permissions: RWD/RWD/P)
...
describe '(Repoman client integration) Test Case 8: Make public module A (User A is not owner but belongs to User group A which is not set on module, permissions: RWD/RWD/P)' do
  ...
  context "Usergroup #{user_data[:another_usergroup]}, user #{user_data[:another_user]} and RWD/RWD/P permissions" do
  ...
  context "Usergroup #{user_data[:usergroup]}, user #{user_data[:user]} and RWD/RWD/RP permissions" do
    include_context 'Check module permissions', dtk_common, user_data[:module_name], 'RWD/RWD/RP'
  end

# Test Case 9: NEG - Make public module A (User A is not owner but belongs to User group A which is set on module, permissions: RWDP/RWD/None)
...
describe "(Repoman client integration) Test Case 9: NEG - Make public module A (User A is not owner but belongs to User group A which is set on module, permissions: RWDP/RWD/None)" do
	...
	context "Usergroup #{user_data[:usergroup]}, user #{user_data[:another_user]} and RWDP/RWD/None permissions" do
  ...
  context "Usergroup #{user_data[:usergroup]}, user #{user_data[:user]} and RWDP/RWD/None permissions" do
  	include_context "Check module permissions", dtk_common, user_data[:module_name], "RWDP/RWD/"
	end

...
if dtk_common.check_if_assembly_exists(assembly_id)
	...
	if !set_attributes_array.include? false
  		assembly_converged = dtk_common.converge_assembly(assembly_id, 50)
  		if assembly_converged == true
    		puts "#{assembly_template} assembly deployed!"
  		else
    		puts "[ERROR] #{assembly_template} assembly was not deployed successfully!"
  		end
	else
  		puts "[ERROR] Some of the attributes are not set correctly. Will not proceed with converge process!"
	end
else
	puts "[ERROR] #{assembly_template} assembly is not staged and therefore deployment of DTK artifacts will not continue!"
end

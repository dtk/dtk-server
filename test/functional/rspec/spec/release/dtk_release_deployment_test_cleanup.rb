...
services = dtk_common.list_matched_success_service("dtk_release_deployment_test")
services.each do |s|
  dtk_common.delete_and_destroy_service(s['id'])
end

failed_services = dtk_common.list_matched_failed_service('dtk_release_deployment_test')

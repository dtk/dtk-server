...
module DTK; class CommandAndControlAdapter::Ec2
  module Reified; class Target
    class Violation
      class InvalidIAMRole < self
        def initialize(role_name)
          @role_name = role_name
        end

        def description(_opts = {})
          component_ref = "#{Target::ComponentType.name(:role)}[#{@role_name}]"
          "IAM role name '#{@role_name}' specified on component '#{component_ref}' is invalid" 
        end
      end
      ...
      class MissingCredential < self
        def initialize(component_type, component_title, attribute_name)
        ...
        def description(_opts = {})
          component_ref = "#{Target::ComponentType.name(@component_type)}[#{@component_title}]"
          "The AWS credential '#{@attribute_name}' on component '#{component_ref}' is missing"
        end
      end

  ...
  class IncrementalGenerator < IGBase
    ...
    class LinkDef < IGBase::LinkDef
      def generate(aug_link_def,opts={})
        ...
        link_defs = generate__link_defs(aug_link_def)
      ...
      def merge_fragment!(full_hash,fragment,context={})
        ...
        merge_fragment__link_defs!(full_hash,fragment['link_defs'],context)
        full_hash
      end

     private
      def generate__link_defs(aug_link_def)
        ...
        aug_link_def[:link_def_links].inject(PossibleLinks.new) do |pl,link_def_link|
          cmp,link = choice_info__link_defs(aug_link_def,ObjectWrapper.new(link_def_link))
          pl.deep_merge(cmp,link)
        end
      end

      def merge_fragment__link_defs!(full_hash,fragment,context={})
        ret = full_hash
      ...
      #returns cmp,link
      def choice_info__link_defs(link_def,link_def_link)
        link = Link.new

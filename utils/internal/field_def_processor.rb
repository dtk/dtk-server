...
pp [:cmp_attrs_objs,cmp_attrs_objs]
      convert_to_field_def_form(cmp_attrs_objs)
    end
    private 
    def self.convert_to_field_def_form(cmp_attrs_objs)
...
=end
    ...
    def self.hidden_fields(cmp_attrs_objs)
      HiddenFields.map do |hf|
        {hf.keys.first => Aux::ordered_hash_subset(hf.values.first,[:required,:type])}
      end
    end

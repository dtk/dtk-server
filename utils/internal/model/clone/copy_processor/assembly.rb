  ...
  class Clone
    class CopyProcessor
      class Assembly < self
        ...
        attr_reader :project,:module_global_refs
       ...
       private
        def initialize(target_obj,source_obj,opts={})
          ...
          @module_global_refs = get_module_global_refs(source_obj)
        end

        def get_module_global_refs(source_obj)
          sp_hash = {
            :cols => [:id],
            :filter => [:eq,:id,source_obj.get_field?(:module_branch_id)]
          }
          Model.get_obj(source_obj.model_handle(:module_branch),sp_hash).get_module_global_refs()
        end

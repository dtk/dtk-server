  ...
  module ParseLogAdapter
    class Puppet
     ...
     public
      ...
      class ErrorTemplate < ErrorPuppetLog 
        def self.isa?(segments_from_error)
       ...
       private
        def parse!(segments_from_error,prev_segment)
          ...
          if segments_from_error.size > 2 
            ...
            if line =~ /INFO: error: Puppet::Mixin::Template::TemplateError: (.+) for #/
              detail = $1
            end
          end
      ...
      class ErrorService < ErrorPuppetLog 
        def self.isa?(segments_from_error)
       ...
       private
        def parse!(segments_from_error,prev_segment)
          ...
          if segment.line =~ /INFO: error: Puppet::Exceptions::Exec/
      ...
      class ErrorRecipe < ErrorPuppetLog 
        def self.isa?(segments_from_error)
       ...
       private
        ...
        def parse!(segments_from_error,prev_segment)
          ...
          if line =~ Regexp.new("#{RecipeCache}([^/]+)/recipes/([^:]+):([0-9]+):in `from_file'")
            ...
            @error_file_ref = PuppetFileRef.recipe(cookbook,recipe_filename)
          end
          @error_detail = segments_from_error.first.aux_data.first
        end
      end

      class ErrorMissingRecipe < ErrorPuppetLog 
        def self.isa?(segments_from_error)
          return nil unless segments_from_error.size > 1
      ...
      class PuppetFileRef < HashObject

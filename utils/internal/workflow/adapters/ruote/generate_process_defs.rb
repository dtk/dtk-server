    ...
    module RuoteGenerateProcessDefs
      ...
      def decomposition(task,context)
        ...
        if action.kind_of?(Task::Action::PowerOnNode)
          ...
          sync_agent_code = participant_executable_action(:sync_agent_code,task,context,:task_type => "sync_agent_code") if task[:executable_action][:node][:inter_node_stage] == "_1"
